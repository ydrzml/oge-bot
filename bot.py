...
async def show_regular_slots(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, s in enumerate(slots):
        ...
        keyboard.append([InlineKeyboardButton(label, callback_data=f"regular_{i}")])
    keyboard.append([InlineKeyboardButton("◀️ Назад", callback_data="back")])

    context.user_data["regular_slots"] = slots  # сохраняем для доступа по индексу
...
async def regular_slot_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    idx = int(query.data.replace("regular_", ""))
    slots = context.user_data.get("regular_slots", [])
...
async def show_my_bookings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем данные записей в user_data по индексу — обходим лимит Telegram 64 байта
    ...
    for i, b in enumerate(bookings):
        ...
        keyboard.append([InlineKeyboardButton(
            f"❌ {type_icon} {b['slot_label']}", callback_data=f"cancel_{i}"
        )])
    keyboard.append([InlineKeyboardButton("◀️ Назад", callback_data="back")])
    await query.edit_message_text(
...
async def do_cancel_booking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    idx = int(query.data.replace("cancel_", ""))
